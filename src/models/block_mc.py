...
def is_cpu_based_device(tested_device):
    return tested_device == torch.device('cpu') or tested_device == torch.device('mps:0')

def add_grid_cache(flow): # TODO Patch for MPS
    device_id = -1 if is_cpu_based_device(flow.device) else flow.device.index
...
def torch_warp(feature, flow):
    device_id = -1 if is_cpu_based_device(feature.device) else feature.device.index
    add_grid_cache(flow)
...
def block_mc_func(im, flow):
    if not CUSTOMIZED_CUDA:
        back_to_mps = im.device == torch.device('mps:0') or flow.device == torch.device('mps:0')
        if back_to_mps:
            im = im.to(torch.device('cpu'))
            flow = flow.to(torch.device('cpu'))
        result = flow_warp(im, flow)
        if back_to_mps:
            result = result.to(torch.device('mps'))
        return result
    with torch.no_grad():
        B, C, H, W = im.size()
        out = torch.empty_like(im)
        block_mc_forward(out, im, flow, B, C, H, W) # Custom cuda implementation...
        return out
